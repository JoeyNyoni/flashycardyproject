      ...
      {userDecks.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {userDecks.map((deck) => (
            <li
              key={deck.id}
              className="rounded-lg border border-border bg-card p-5 flex flex-col gap-2 hover:bg-accent transition-colors"
            >
              <span className="font-semibold text-card-foreground">{deck.name}</span>
              {deck.description && (
                <span className="text-sm text-muted-foreground line-clamp-2">{deck.description}</span>
              )}
            </li>
          ))}
        </ul>
